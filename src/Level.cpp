...
Level::Level(Backend *backend) noexcept : Scene(backend) {
  ...
  map.push_back(
      ...
      "sDE......ttt................8");
  map.push_back(
      "1......24.....s..FG..................b............BC..FG..FG............"
      "......FG..FG..BC.................56667..........23333334.............."
      "23334....................e..8");
  map.push_back(
      "1p.....8A..23334.DE............................z..DE.zDE..DE............"
      "......DE..DE..DE...z....z.......................56666667....g........."
      "56667...23334...............8");
  map.push_back(
      "33333339A..8999933334........23333333333333333333333333333334..........."
...
bool ResolveCollision(Rectangle &r_dynamic, Vector2<float> &velocity,
                      const float deltaTime, Rectangle &r_static) noexcept {
  Vector2<float> contact_point, contact_normal;
  float contact_time = 0.f;

  if (r_static.Intersects(r_dynamic, velocity, deltaTime, contact_point,
                          contact_normal, contact_time)) {
    velocity += contact_normal *
                Vector2<float>(std::abs(velocity.x), std::abs(velocity.y)) *
                (1 - contact_time);
    return true;
  }
...
bool ResolveDynamicCollision(Rectangle &r_dynamic, Vector2<float> &velocity,
                             const float deltaTime, Rectangle &r_static,
                             Vector2<float> &dyn_velocity) noexcept {
  Vector2<float> contact_point, contact_normal;
  float contact_time = 0.f;

  if (r_static.Intersects(dyn_velocity, r_dynamic, velocity, deltaTime,
...
void Level::Update(float deltaTime) noexcept {
  ...
  for (int i = 0; i < entities.size(); i++) {
    ...
    if (entity->entityBox.pos.x < -entity->entityBox.size.x ||
        ...
        entity->entityBox.pos.y >
            this->map.tiles[0].size() * this->map.tileSize) {
      if (i == 0)
        entity->health = 0;
      else {
        dead.push_back(entity);
    ...
    if (entity->boundsReset) {
      if (entity->entityBox.pos.x <
              this->camera.pos.x - (this->camera.size.x / 3) ||
          entity->entityBox.pos.x > this->camera.pos.x + this->camera.size.x +
                                        (this->camera.size.x / 3) ||
          entity->entityBox.pos.y <
              this->camera.pos.y - (this->camera.size.y / 3) ||
          entity->entityBox.pos.y > this->camera.pos.y + this->camera.size.y +
                                        (this->camera.size.y / 3))
        continue;
    }
    ...
    if (entity->hasCollision) {
      const Rectangle next = {entity->entityBox.pos +
                                  (entity->velocity * deltaTime),
                              entity->entityBox.size};
      ...
      if (next.pos.x >= entity->entityBox.pos.x) {
        ...
        endX = static_cast<int>(entity->entityBox.pos.x +
                                entity->entityBox.size.x) /
               this->map.tileSize;
      ...
      if (next.pos.y >= entity->entityBox.pos.y) {
        ...
        endY = static_cast<int>(entity->entityBox.pos.y +
                                entity->entityBox.size.y) /
               this->map.tileSize;
      ...
      startY =
          std::min(startY, static_cast<int>(this->map.tiles[0].size() - 1));
      ...
      std::vector<std::tuple<float, Rectangle, Tile, Vector2<float>>>
          collisions;

      for (int x = startX; x <= endX; x++) {
        for (int y = startY; y <= endY; y++) {
          const Rectangle rect = {
              Vector2<float>(x * this->map.tileSize, y * this->map.tileSize),
              Vector2<float>(this->map.tileSize, this->map.tileSize)};

          if (this->map.tiles[x][y].type != Tile::Type::Passable &&
              rect.Intersects(entity->entityBox, entity->velocity, deltaTime,
                              cp, cn, ct))
      ...
      for (int i = 0; i < collisions.size(); i++) {
        ...
        // resolve collision
        if (entity->OnCollide(i, std::get<2>(c), std::get<3>(c),
                              std::get<1>(c)))
          ResolveCollision(entity->entityBox, entity->velocity, deltaTime,
                           std::get<1>(c));
      }
    }
    ...
    if (entity->hasEntityCollision) {
      ...
      for (int j = 0; j < this->entities.size(); j++) {
        ...
        if (e != entity) {
          if (e->entityBox.Intersects(e->velocity, entity->entityBox,
                                      entity->velocity, deltaTime, cp, cn, ct))
      ...
      std::sort(collisions.begin(), collisions.end(),
                [](const std::tuple<float, int, Vector2<float>> &a,
                   const std::tuple<float, int, Vector2<float>> &b) {
                  return std::get<0>(a) < std::get<0>(b);
                });
      ...
      for (auto &c : collisions) {
        ...
        if (entity->OnCollide(entities[std::get<1>(c)], std::get<2>(c)))
          ResolveDynamicCollision(entity->entityBox, entity->velocity,
                                  deltaTime,
                                  entities[std::get<1>(c)]->entityBox,
                                  entities[std::get<1>(c)]->velocity);
      }
    }
  ...
  if (this->entities.size() > 0) {
    this->camera.pos = this->entities[0]->entityBox.pos +
                       (this->entities[0]->entityBox.size / 2) -
                       (this->camera.size / 2);

    if (this->camera.pos.x < 0)
      this->camera.pos.x = 0;

    if (this->camera.pos.x + this->camera.size.x >
        this->map.tiles.size() * this->map.tileSize)
      this->camera.pos.x =
          this->map.tiles.size() * this->map.tileSize - this->camera.size.x;

    if (this->camera.pos.y < 0)
      this->camera.pos.y = 0;

    if (this->camera.pos.y + this->camera.size.y >
        this->map.tiles[0].size() * this->map.tileSize)
      this->camera.pos.y =
          this->map.tiles[0].size() * this->map.tileSize - this->camera.size.y;
...
void Level::Render() noexcept {
  ...
  for (int x = startX - 1; x <= endX; x++) {
    ...
    for (int y = startY - 1; y <= endY; y++) {
      ...
      this->backend->DrawTexture(this->spriteMap, this->map.tiles[x][y].sprite,
                                 rect);
    }
  }
  ...
  for (auto entity : this->entities) {
    if (entity->entityBox.pos.x >=
            this->camera.pos.x - entity->entityBox.size.x &&
        entity->entityBox.pos.x <= this->camera.pos.x + this->camera.size.x +
                                       entity->entityBox.size.x &&
        entity->entityBox.pos.y >=
            this->camera.pos.y - entity->entityBox.size.y &&
        entity->entityBox.pos.y <=
            this->camera.pos.y + this->camera.size.y + entity->entityBox.size.y)
  ...
  const Rectangle gold_text = {{70.f, 20.f},
                               {24.f * ceilf(gold_string.length() + 2), 24.f}};
  ...
  // render score stats
  const Rectangle score_text = {
      {width - 200.f, 10.f}, {12.f * ceilf(score_string.length() + 8), 24.f}};

  this->RenderText(std::string("SCORE:`") + score_string, score_text);
}

void Level::RenderText(std::string_view text,
