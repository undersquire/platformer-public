...
Bat::Bat(Backend *backend, Vector2<float> &spawn) noexcept : Entity(backend, EntityType::Bat)
{
...
bool Bat::OnCollide(int i, Tile &tile, const Vector2<float> &contact, const Rectangle &tile_pos) noexcept
{
    if (this->deathFalling)
    {
        if (contact.y == -1)
