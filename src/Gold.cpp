...
Gold::Gold(Backend *backend, Vector2<float> &spawn) noexcept : Entity(backend, EntityType::GoldItem)
{
...
void Gold::Update(float) noexcept
{
...
void Gold::Render(const Rectangle &camera) noexcept
{
    ...
    // render sprite
    this->backend->DrawTexture(this->spriteMap, *this->spriteSheet.GetFrame(), dest, 0.f, center, 0);
}