...
Backend::Backend(const char *title, unsigned width, unsigned height)
  ...
  this->renderer =
      SDL_CreateRenderer((SDL_Window *)this->window, -1,
...
bool Backend::IsRunning() const noexcept { return this->running; }
...
AssetManager &Backend::GetAssetManager() { return this->assetManager; }
...
void Backend::ClearDisplay(const Color &color) noexcept {
  SDL_SetRenderDrawColor((SDL_Renderer *)this->renderer, color.r, color.g,
                         color.b, color.a);
  SDL_RenderClear((SDL_Renderer *)this->renderer);
}

void Backend::DrawLine(const Vector2<float> &p1, const Vector2<float> &p2,
...
void Backend::DrawRectangle(const Rectangle &rect,
                            const Color &color) noexcept {
  SDL_SetRenderDrawColor((SDL_Renderer *)this->renderer, color.r, color.g,
                         color.b, color.a);
...
void Backend::FillRectangle(const Rectangle &rect,
                            const Color &color) noexcept {
  SDL_SetRenderDrawColor((SDL_Renderer *)this->renderer, color.r, color.g,
                         color.b, color.a);
...
Backend::Texture *Backend::InitTexture(void *textureData) noexcept {
  SDL_Texture *texture = SDL_CreateTextureFromSurface(
      (SDL_Renderer *)this->renderer, (SDL_Surface *)textureData);
...
void Backend::DrawTexture(Backend::Texture *texture, const Rectangle &source,
                          const Rectangle &destination) noexcept {
  if (texture == nullptr)
    return;

  SDL_Rect src = {
      static_cast<int>(source.pos.x), static_cast<int>(source.pos.y),
      static_cast<int>(source.size.x), static_cast<int>(source.size.y)};

  SDL_RenderCopyF((SDL_Renderer *)this->renderer, (SDL_Texture *)texture->data,
                  &src, (const SDL_FRect *)&destination);
}

void Backend::DrawTexture(Backend::Texture *texture, const Rectangle &source,
                          const Rectangle &destination, const double angle,
                          const Vector2<float> &center,
                          const unsigned flip) noexcept {
  if (texture == nullptr)
    return;
  ...
  SDL_RenderCopyExF((SDL_Renderer *)this->renderer,
                    (SDL_Texture *)texture->data, &src,
                    (const SDL_FRect *)&destination, angle,
...
void Backend::AddCachedTexture(Backend::Texture *texture,
...
void Backend::GetMouseState(MouseState &state) noexcept {
  ...
  SDL_RenderWindowToLogical((SDL_Renderer *)this->renderer, rawMouseX,
                            rawMouseY, &mouseX, &mouseY);
