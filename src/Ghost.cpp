...
Ghost::Ghost(Backend *backend, Vector2<float> &spawn) noexcept : Entity(backend, EntityType::Ghost)
{
...
void Ghost::Update(float deltaTime) noexcept
{
    ...
    {
        if (this->entityBox.pos.x > this->target->entityBox.pos.x && this->target->directionX == -1)
        {
            this->moving = true;
            this->directionX = -1;
        }
        else if (this->entityBox.pos.x < this->target->entityBox.pos.x && this->target->directionX == 1)
        {
            this->moving = true;
            this->directionX = 1;
        }
