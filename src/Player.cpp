...
void Player::Update(float deltaTime) noexcept {
  if (this->health > 0) {
    ...
    if (backend->IsKeyDown(Backend::KeyCode::LEFT) ||
        backend->IsKeyDown(Backend::KeyCode::A)) {
      this->directionX = -1;
    ...
    if (backend->IsKeyDown(Backend::KeyCode::RIGHT) ||
        backend->IsKeyDown(Backend::KeyCode::D)) {
      this->directionX = 1;
...
bool Player::OnCollide(int, Tile &tile, const Vector2<float> &contact,
  ...
  if (this->health > 0) {
    ...
    if (contact.y == 1) // vertical collision
    {
      ...
      if (this->isOnGround && tile.type == Tile::Type::Platform) {
        if (this->backend->IsKeyDown(Backend::KeyCode::DOWN) ||
            this->backend->IsKeyDown(Backend::KeyCode::S))
          return false;
      }
