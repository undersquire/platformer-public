...
LevelStats::LevelStats(Backend *backend, ScoreKeeper scoreKeeper) noexcept
    : Scene(backend), scoreKeeper(scoreKeeper) {
...
void LevelStats::Render() noexcept {
  ...
  // render gold stats
  Rectangle gold_text = {{0.f, height / 2},
                         {24.f * ceilf(gold_string.length() + 8), 36.f}};
  ...
  // render score stats
  Rectangle score_text = {{0.f, (height / 2) + 100.f},
                          {24.f * ceilf(score_string.length() + 8), 36.f}};
...
void LevelStats::RenderText(std::string_view text,
